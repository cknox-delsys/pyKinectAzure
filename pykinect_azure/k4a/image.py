...
class Image:
	...
	def __init__(self, image_handle=None, k4a_dll_obj=None):
	...
	def reset(self):
		if self.is_valid():
			self._k4a.k4a_image_release(self._handle)
			self._handle = None

	def create(self, image_format,width_pixels,height_pixels,stride_bytes):
		handle = self._k4a.k4a_image_t()
		self._k4a.VERIFY(self._k4a.k4a_image_create(image_format,width_pixels,height_pixels,stride_bytes,handle),"Create image failed!")

		return Image(handle, self._k4a)
	...
	def get_buffer(self):
		...
		return self._k4a.k4a_image_get_buffer(self._handle)
		
	def get_size(self):
		if not self.is_valid():
			return None

		return int(self._k4a.k4a_image_get_size(self._handle))

	def get_format(self):
		if not self.is_valid():
			return None

		return int(self._k4a.k4a_image_get_format(self._handle))

	def get_width_pixels(self):
		if not self.is_valid():
			return None

		return int(self._k4a.k4a_image_get_width_pixels(self._handle))

	def get_height_pixels(self):
		if not self.is_valid():
			return None

		return int(self._k4a.k4a_image_get_height_pixels(self._handle))

	def get_stride_bytes(self):
		return int(self._k4a.k4a_image_get_stride_bytes(self._handle))
	...
	def to_numpy(self):
		...
		if image_format == self._k4a.K4A_IMAGE_FORMAT_COLOR_MJPG:
			return True, cv2.imdecode(np.frombuffer(buffer_array, dtype=np.uint8).copy(), -1)
		elif image_format == self._k4a.K4A_IMAGE_FORMAT_COLOR_NV12:
		...
		elif image_format == self._k4a.K4A_IMAGE_FORMAT_COLOR_YUY2:
		...
		elif image_format == self._k4a.K4A_IMAGE_FORMAT_COLOR_BGRA32:
			return True, np.frombuffer(buffer_array, dtype=np.uint8).copy().reshape(image_height,image_width,4)
		elif image_format == self._k4a.K4A_IMAGE_FORMAT_DEPTH16:
		...
		elif image_format == self._k4a.K4A_IMAGE_FORMAT_IR16:
